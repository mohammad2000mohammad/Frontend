...
const OrdersPage = () => {
    ...
    const fetchOrders = async () => {
      ...
      try {
        const response = await fetch(
          `https://backend-v6sz.onrender.com/api/orders/search?searchTerm=${searchTerm}&page=${page}&limit=10&dateFilter=${dateFilter}&statusFilter=${statusFilter}`, {
  ...
  const updateOrderStatus = (orderId, newStatus) => {
    ...
    fetch(`https://backend-v6sz.onrender.com/api/orders/${trimmedOrderId}/status`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${token}`, // Include token in the Authorization header
